...
def adicionar_tarefa():
    ...
    # Checa o campo Repetir (se presente)
    repetir = dados_tarefa.get('repetir', '0')
    id_usuario = dados_tarefa['id_usu']
    # Valor padrão se não for passado
    concluida = dados_tarefa.get('concluida', False)

    # Processar as tarefas com base nas opções de repetição
    # Usando sua função de processamento
    resultado = processar_dados_tarefa(dados_tarefa)
    if resultado['erro']:
        # Se ocorrer erro na função de processamento, retornamos a resposta de erro
...
def get_usuario_por_id(usuario_id):
    try:
        # Chama a função para consultar o usuário
        ...
        if usuario:
            ...
            data = {
                ...
                "celular": usuario[3],
                "email": usuario[4],
                # Supondo que este campo contenha a URL da imagem
...
def buscar_usuario_por_id():
    ...
    try:
        ...
        if usuario:
            ...
            # Verifica se os primeiros 5 campos não têm valor None
            if None not in usuario[:5]:
                data = {
                    "id": usuario[0],
                    "nome": usuario[1],
                    "data_nascimento": usuario[2],
                    "celular": usuario[3],
                    "email": usuario[4],
                    # Se não houver imagem, atribui None
                    "imagem_perfil": usuario[5] if usuario[5] is not None else None
                }
                return jsonify(data), 200
            else:
...
def listar_notificacoes_route():
    try:
        ...
        # Buscar as notificações filtradas pela data e horário correto
        notificacoes = listar_notificacoes(user_id)

        # Verifica se retornou erro
        if isinstance(notificacoes, dict) and notificacoes.get('erro', False):
        ...
        # Log para verificar as notificações que estão sendo retornadas
