...
# >>> Importa o arquivo com a função que insere as informações no banco de dados <<<
import Gravar_BD
from Gravar_BD import gravar_tarefas, gravar_notificacao
...
def processar_dados_log(dados):
    ...
    if mensagens_erro:
        ...
        login_val = confere_dados_com_banco(
            dados['email_log'], dados['senha_log'])
        if login_val:
            # Login bem-sucedido
...
def processar_dados_tarefa(dados):
    ...
    cria notificações para serem enviadas nos horários corretos.
    """
    ...
    dados['ID_PAI'] = id_pai  # Adicionar o ID_PAI à tarefa original
    ...
    if dados['Repetir'] in ['1', '2', '3', '4']:  # Valores para repetir
    ...
    # Gravar as tarefas no banco de dados e criar notificações
    for tarefa in tarefas_para_gravar:
        resultado = gravar_tarefas(
            tarefa['Cor'],
            tarefa['Titulo'],
            tarefa['Inicio'],
            tarefa['Termino'],
        ...
        if tarefa['Notific'] in [True, 'True', '1']:  # Verifica se notificação está ativada
            try:
                id_tarefa = resultado['id_tarefa']
                inicio_tarefa = datetime.strptime(tarefa['Inicio'], '%Y-%m-%d %H:%M')

                # Criar apenas duas notificações: uma para 1 hora antes e outra para 15 minutos antes
                horarios = [
                    inicio_tarefa - timedelta(hours=1),  # Notificação de 1 hora antes
                    inicio_tarefa - timedelta(minutes=15)  # Notificação de 15 minutos antes
                ]
                mensagens = [
                ...
                for i, horario in enumerate(horarios):
                    # Grava a notificação para ser enviada em algum momento no futuro
                    notificacao_result = gravar_notificacao(
                        tarefa['ID_Usu'],
                        id_tarefa,
                        mensagens[i],
                        horario.strftime('%Y-%m-%d %H:%M'),
                        tarefa['Repetir']  # Passando a repetição para a função gravar_notificacao
                    )
                    if notificacao_result.get("erro"):
                        raise Exception(notificacao_result.get("mensagem"))
                
                logger.info(f"Notificações gravadas com sucesso para a tarefa ID {id_tarefa}.")
            
            except Exception as e:
                return {'erro': True, 'mensagem': f'Erro ao gravar notificações para tarefa ID {id_tarefa}: {str(e)}'}

    return {'erro': False, 'mensagem': 'Tarefas e notificações gravadas com sucesso!'}

# Função para processar as notificações existentes
# Autor: Emily
# Data: 09/12/2024
def verificar_notificacao_existente(id_tarefa, horario_notificacao):
    """
    Verifica se já existe uma notificação para a tarefa e data específica.
    """
    sql = "SELECT COUNT(*) FROM notificacao WHERE ID_Taf = %s AND Data_Hora = %s"
    try:
        with conexao.conectar() as conex:
            with conex.cursor() as cursor:
                cursor.execute(
                    sql, (id_tarefa, horario_notificacao.strftime('%Y-%m-%d %H:%M:%S')))
                resultado = cursor.fetchone()
                
                # Verifica se o resultado não é None e retorna True se a notificação já existe
                if resultado and resultado[0] > 0:
                    return True
                return False
    except Exception as e:
        logger.error(f"Erro ao verificar notificação existente: {e}", exc_info=True)
        return False
