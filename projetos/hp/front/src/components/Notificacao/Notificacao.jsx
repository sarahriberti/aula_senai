...
const Notificacao = () => {
    ...
    const confirmarDeletar = async () => {
        try {
            const response = await fetch(`http://10.135.60.33:8085/excluir_notificacao?id=${notificacaoParaDeletar}`, {
                method: 'DELETE',
            });
            ...
            {modalConfirmacao && (
                <div className="modal-confirmacao">
                    <div className="modal-content">
                        <h3>Você tem certeza que deseja deletar essa notificação?</h3>
                        <div className="modal-buttons">
                            <button onClick={confirmarDeletar}>Confirmar</button>
                            <button onClick={cancelarDeletar}>Cancelar</button>
                        </div>
                    </div>
                </div>
            )}
